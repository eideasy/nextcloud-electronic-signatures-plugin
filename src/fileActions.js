...
function addCustomFileActions() {
  const displayName = () => translate(config.appId, 'Signing');
  const getActionNodes = actionArgument => {
    if (actionArgument && Array.isArray(actionArgument.nodes)) {
      return actionArgument.nodes;
    }
  ...
  const fileAction = {
    ...
    async exec(actionArgument) {
      const file = getActionNodes(actionArgument)[0];
    ...
    enabled(actionArgument) {
      return getActionNodes(actionArgument).length === 1;
    },
    order: 1,
  };
